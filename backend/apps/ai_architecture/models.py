...
class DrawingStylePreset(TimeStampedModel):
    """
    Admin-configurable templates for different architectural drawing types.
    The system matches user requests to presets via keywords, then uses the
    preset's template to guide Claude's prompt engineering for Gemini image generation.
    """
    CATEGORY_CHOICES = [
    ...
    name = models.CharField(max_length=100, help_text="Display name, e.g. 'Floor Plan'")
    category = models.CharField(max_length=20, choices=CATEGORY_CHOICES, default='OTHER')
    keywords = models.TextField(
        help_text="Comma-separated trigger keywords, e.g. 'floor plan, floorplan, room layout'"
    )
    prompt_template = models.TextField(
        help_text=(
            "Template injected into Claude's prompt-engineering step. "
            "Use {user_request} as placeholder for the original user message."
        ),
        default=(
            "Create a highly detailed architectural {category} based on this request: {user_request}. "
            "Use clean black-and-white line work, professional drafting style, labeled rooms with "
            "dimensions, and a top-down orthographic view. Include a title block and north arrow."
        )
    )
    negative_prompt = models.TextField(
        blank=True,
        default="blurry, low quality, realistic photo, 3d render, watermark, text overlay, cartoonish",
        help_text="What to avoid in the generated image."
    )
    style_tokens = models.TextField(
        blank=True,
        default="architectural blueprint, technical drawing, clean lines, professional CAD",
        help_text="Extra style keywords appended to every prompt of this type."
    )
    guidance_scale = models.FloatField(
        default=7.5,
        help_text="Gemini guidance scale (higher = more prompt adherence, 3-15 typical)."
    )
    is_active = models.BooleanField(default=True)
    priority = models.IntegerField(
        default=0,
        help_text="Higher priority presets are matched first when keywords overlap."
    )
...
class ImageFeedback(TimeStampedModel):
    """
    User feedback on AI-generated images. Positive feedback prompts
    are recycled as examples when generating new images of the same type.
    """
    RATING_CHOICES = [
        (1, 'Poor'),
        (2, 'Below Average'),
    ...
    original_prompt = models.TextField(
        help_text="The exact prompt that was sent to Gemini for this image."
    )
    preset_used = models.ForeignKey(
        DrawingStylePreset, on_delete=models.SET_NULL, null=True, blank=True,
        help_text="Which style preset was active when this image was generated."
    )
    ...
    def __str__(self):
        return f"Rating {self.rating}/5 by {self.user.username}"
