...
function resolveAssetsCdnUrl(): string {
  const viteEnv = (import.meta as any)?.env?.VITE_PASCAL_ASSETS_CDN_URL
  if (typeof viteEnv === 'string' && viteEnv.trim()) {
    return viteEnv.trim()
  }

  const nodeEnv = (globalThis as any)?.process?.env?.NEXT_PUBLIC_ASSETS_CDN_URL
...
export const ASSETS_CDN_URL = resolveAssetsCdnUrl()
