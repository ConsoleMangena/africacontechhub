...
export const StairTool: React.FC = () => {
  ...
  const resolveTargetLevelId = () => {
    const { nodes } = useScene.getState()
    const selection = useViewer.getState().selection
    if (selection.levelId && nodes[selection.levelId]?.type === 'level') {
      return selection.levelId as LevelNode['id']
    }
    const selectedBuilding = selection.buildingId ? (nodes[selection.buildingId as AnyNode['id']] as any) : null
    if (selectedBuilding?.type === 'building' && Array.isArray(selectedBuilding.children)) {
      const levelFromBuilding = selectedBuilding.children.find(
        (childId: string) => nodes[childId as AnyNode['id']]?.type === 'level',
      )
      if (levelFromBuilding) {
        return levelFromBuilding as LevelNode['id']
      }
    }
    const fallback = Object.values(nodes).find((node: any) => node?.type === 'level') as
      | { id?: string }
      | undefined
    return (fallback?.id as LevelNode['id'] | undefined) ?? null
  }
  ...
  useEffect(() => {
    ...
    const onGridClick = (event: GridEvent) => {
      ...
      const currentLevelId = resolveTargetLevelId()
      if (!currentLevelId) {
        window.dispatchEvent(
          new CustomEvent('pascal-tool-error', { detail: 'Stair placement failed: no active level selected' }),
        )
        return
      }
      ...
      const stairId = commitStairPlacement(currentLevelId, [gridX, y, gridZ], rotationRef.current)
      emitter.emit('camera-controls:focus', { nodeId: stairId })
    }
