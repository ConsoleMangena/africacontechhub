...
export function useDraftNode(): DraftNodeHandle {
  ...
  const commit = useCallback((finalUpdate: Partial<ItemNode>): string | null => {
    ...
    const parentId = (newParentId ??
      // Prefer the viewer selection, but don't let a null selection break commits mid-placement.
      useViewer.getState().selection.levelId ??
      // Fall back to the draft's existing parent (created from level selection).
      (draft.parentId as AnyNodeId | null)) as AnyNodeId
