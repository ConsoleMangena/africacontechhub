...
export function createWallOnCurrentLevel(
  ...
  const selectedBuilding = viewerSelection.buildingId
    ? (nodes[viewerSelection.buildingId] as any)
    : null
  const levelFromSelectedBuilding =
    selectedBuilding?.type === 'building' && Array.isArray(selectedBuilding.children)
      ? (selectedBuilding.children.find((childId: string) => nodes[childId]?.type === 'level') as
          | string
          | undefined)
      : undefined
  const currentLevelId =
    (viewerSelection.levelId && nodes[viewerSelection.levelId]?.type === 'level'
      ? viewerSelection.levelId
      : null) ??
    levelFromSelectedBuilding ??
