...
export const WallTool: React.FC = () => {
  ...
  const notifyWallError = (text: string) => {
    window.dispatchEvent(new CustomEvent('pascal-tool-error', { detail: text }))
  }
    ...
    const onGridClick = (event: GridEvent) => {
      ...
      if (buildingState.current === 0) {
        ...
        if (dx * dx + dz * dz < 0.01 * 0.01) {
          notifyWallError('End point is too close. Move cursor and click again to finish wall.')
        ...
        const wall = createWallOnCurrentLevel([startingPoint.current.x, startingPoint.current.z], snappedEnd)
        if (!wall) {
          notifyWallError('Wall placement failed: no active level selected')
        } else {
