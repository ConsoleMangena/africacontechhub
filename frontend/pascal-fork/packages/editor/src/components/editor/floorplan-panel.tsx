...
function FloorplanZoneLabel({
  ...
  const hasValidCentroid = Number.isFinite(centroid.x) && Number.isFinite(centroid.y)
  ...
  return (
    <>
      {zonePolygons.map(({ zone, polygon }) => {
        ...
        if (!Number.isFinite(rawCentroid.x) || !Number.isFinite(rawCentroid.y)) return null
