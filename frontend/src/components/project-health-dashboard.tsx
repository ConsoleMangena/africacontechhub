...
function HealthIndicator({ label, status, value, icon, details }: HealthIndicatorProps) {
  ...
  return (
    <div className={cn('rounded-md border p-1.5 sm:p-2 transition-all hover:shadow-sm', config.bg, config.border)}>
      <div className="flex items-center gap-1.5 sm:gap-2">
        <div className={cn('h-6 w-6 sm:h-8 sm:w-8 rounded-md bg-white flex items-center justify-center shrink-0', config.icon)}>
          <Icon name={icon} size={14} className="sm:hidden" />
          <Icon name={icon} size={16} className="hidden sm:block" />
        </div>
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-1 sm:gap-1.5 mb-0.5 flex-wrap">
            <p className="text-[9px] sm:text-[10px] font-semibold text-slate-600 uppercase tracking-wide">{label}</p>
            <Badge className={cn('text-[7px] sm:text-[8px] px-1 py-0 h-3 leading-none', config.badge)}>
              {status === 'good' ? 'On Track' : status === 'warning' ? 'Attention' : 'Critical'}
            </Badge>
          </div>
          <p className={cn('text-xs sm:text-sm font-bold', config.text)}>{value}</p>
          {details && <p className="text-[8px] sm:text-[9px] text-slate-500 mt-0.5 truncate">{details}</p>}
        </div>
      </div>
    </div>
  )
...
export function ProjectHealthDashboard({
  ...
  return (
    <Card className="border-2 border-blue-200 bg-gradient-to-br from-blue-50/50 to-white">
      ...
      <CardContent className="grid grid-cols-1 sm:grid-cols-2 gap-2 px-3 sm:px-4">
        <HealthIndicator
          label="Budget"
          status={budgetStatus}
